...
pub fn handler_ise(ctx: Context<InitializeStakeEntry>) -> Result<()> {
    ...
    user_stake_entry.user_stake_token_account = ctx.accounts.user_stake_token_account.key();
    user_stake_entry.bump = ctx.bumps.user_stake_entry;
    user_stake_entry.balance = 0;

    msg!("Stake entry for a user has been initialized");
